...
def pytorch_slos_output_distribution(
    ...
    U_F = {tuple([0] * m): torch.ones(batch_size, dtype=torch.cfloat)}
    ...
    for idx, p in enumerate(idx_n):
        ...
        U_Fp1 = torch.zeros((batch_size, math.comb(m+idx, idx+1)), dtype=torch.cfloat)
        combinations = []

        for state, amplitude in U_F.items():
            nstate = list(state)
            for i in range(m):
                nstate[i] += 1
                nstate_tuple = list(nstate)
                if nstate_tuple not in combinations:
                    combinations.append(nstate_tuple)
                index = combinations.index(nstate_tuple)
                U_Fp1[:, index] += (unitary[:, i, p] * amplitude *
                                    torch.sqrt(torch.tensor(nstate[i]) / input_state_tensor[p]))
                nstate[i] -= 1

        if idx != len(idx_n) - 1:
            U_F = {tuple(combinations[i]): U_Fp1[:, i] for i in range(len(combinations))}
        elif keep_keys or output_map_func is not None:
            keys = [tuple(combinations[i]) for i in range(len(combinations))]
...
def combination_to_rank(combination: List[int], m:int, n:int,
                        start:int=0, previous_c:int=-1, rank:int=0, to_cache:int=None) -> Tuple[int, int, int]:
    cache_previous_c = -1
    cache_rank = 0
...
def pytorch_slos_output_distribution_nobunching(
            ...
            for idx in range(U_F.shape[-1]):
                ...
                for k in range(m):
                    if pos < i+1 and combination[pos] == k:
                        combination[pos], combination[pos-1] = combination[pos-1], combination[pos]
                        pos += 1
                    else:
                        combination[pos-1] = k
                        #new_idx, _, _ = combination_to_rank(combination, m, i + 1)
                        new_idx, cache_previous_c, cache_rank = combination_to_rank(combination, m, i + 1,
                                                                                    cache_start, cache_previous_c, cache_rank, pos-1)
                        cache_start = pos - 1
    ...
    if output_map_func is not None:
        ...
        keys = [comb_to_state(rank_to_combination(idx, m, n), m) for idx in range(U_Fp1.shape[-1])]
...
if __name__ == "__main__":
    ...
    keys, probs = pytorch_slos_output_distribution_nobunching(unitary, input_state)
